 ...
 * /*
 * Deve receber informações de usuário e senha e compara-las com as da classe. Caso sejam realmente iguais, deve retornar
 * verdadeiro, ou então deve lançar uma nova exceção dizendo qual credencial está errada, tratar essa exceção dentro do
 * próprio método imprimindo o erro em tela e por fim retornar false.
 * Ex:
 * try{
 *      if(<<usuário incorreto>>) { throw new Exception(“Usuário incorreto”); } catch
 * (Exception e) { System.out.println(“Erro”); } }
 *
 * Feito isso, crie uma classe para testar a inicialização de um objeto do tipo
 * Login e que utilize o método fazerLogin, com informações digitadas pelo
 * usuário.
 */
package uj.excecoes;
